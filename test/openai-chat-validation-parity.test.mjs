...
const REJECTIONS = [
  ...
  ['response_format text with a stray json_schema', { response_format: { type: 'text', json_schema: { name: 'x', schema: { type: 'object' } } } }, { param: 'response_format.json_schema', code: 'unknown_parameter', message: "Unknown parameter: 'response_format.json_schema'." }],
  ['response_format json_object with a stray json_schema', { response_format: { type: 'json_object', json_schema: { name: 'x', schema: { type: 'object' } } } }, { param: 'response_format.json_schema', code: 'unknown_parameter', message: "Unknown parameter: 'response_format.json_schema'." }],
  ['a tool function with a whitespace-only name', { tools: [{ type: 'function', function: { name: '   ', parameters: {} } }] }, { param: 'tools[0].function.name', code: 'invalid_value', message: "Invalid 'tools[0].function.name': string does not match pattern. Expected a string that matches the pattern '^[a-zA-Z0-9_-]+$'." }],
  ['a legacy function with a blank name', { functions: [{ name: '', parameters: {} }] }, { param: 'functions[0].name', code: 'empty_string', message: "Invalid 'functions[0].name': empty string. Expected a string with minimum length 1, but got an empty string instead." }],
  ['a legacy function_call naming an undeclared function', { functions: [{ name: 'real', parameters: {} }], function_call: { name: 'never_declared' } }, { param: 'function_call', code: null, message: "Invalid value for 'function_call': no function named 'never_declared' was specified in the 'functions' parameter." }],
  ['tool_choice auto with tools absent', { tool_choice: 'auto' }, { param: 'tool_choice', code: null, message: "Invalid value for 'tool_choice': 'tool_choice' is only allowed when 'tools' are specified." }],
