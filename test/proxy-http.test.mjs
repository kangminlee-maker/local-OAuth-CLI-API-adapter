...
test('a runtime cache hit is reported in the shape the client asked in', async () => {
  ...
  const cached = await startProxyWithBackend({
    name: 'cache-backend',
    model: 'fake-local-model',
    async generate(request) {
      return {
  ...
  try {
    const body = await (await postJsonTo(cached.url, '/v1/messages', {
      model: 'fake-local-model',
      max_tokens: 64,
    ...
    const chat = await (await postJsonTo(cached.url, '/v1/chat/completions', {
      model: 'fake-local-model',
      messages: [{ role: 'user', content: 'hi' }],
    })).json();
