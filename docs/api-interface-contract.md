...
| Field | Supported shape | Handling |
| --- | --- | --- |
| `model` | string | Accepted. Defaults to `codex-app-server` when omitted. Whether it selects the executed model depends on `modelSelection.honorRequestModel`; see Model execution. |
...
| Empty model | A `model` that is empty or whitespace-only is normalized to the omitted-model sentinel, exactly as a missing field is. | This predates `modelSelection.honorRequestModel` and applies with the switch off or on, so an empty value selects the configured model rather than being rejected. Clients that want a specific model must name one. |
| Omitted vs. backend alias | A request that omits `model` is normalized to the sentinel `codex-app-server`, and the backends treat that value as "no model chosen": it selects the configured model rather than being honoured or rejected as a model name. Each backend treats its own alias (`codex-backend`, `claude-code-cli`) the same way. | `GET /v1/models` returns the backend's exposed model — the configured model when one is configured, and the alias only when none is. An alias is therefore not generally what a client discovers; it is accepted as an omission sentinel for compatibility with clients that do echo one. A client that wants a specific model must name a real one. |
