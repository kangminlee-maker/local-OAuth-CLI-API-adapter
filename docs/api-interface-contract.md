...
| `mask` | invalid | optional | On a generation an unknown parameter (400 `unknown_parameter`, measured). On an edit, sent to the backend as the `image_generation` tool's `input_image_mask` (a data URL for base64/path sources, the URL itself for URL sources); never attached as an input image; null is omission. On the diagnostic `app-server` image transport there is no tool declaration: the mask is attached as one more reference image and nothing names it — see the "Images `app-server` transport" row. |
...
| `background` | optional | optional | `transparent`, `opaque`, `auto`. Sent on the `image_generation` tool (`codex-backend` image transport; the `app-server` transport delivers it nowhere — see its row). The backend image model refuses `transparent` (`image_generation_user_error` / `invalid_value`), and that rejection is forwarded, streamed or not. |
...
| Images `app-server` transport | `--codex-image-transport app-server` delivers **no Images API option** to the model. That path starts a Codex app-server turn with the prompt and the attached images only — there is no `image_generation` tool declaration to carry `size`, `quality`, `output_format`, `output_compression`, `background`, `moderation`, `input_fidelity` or `input_image_mask`, and none of them is described in prose either. A `mask` is attached as one more reference image. The response still echoes the request's `background`, `size`, `quality` and `output_format`, none of which reached the model. The options are **accepted, not refused**, on this transport: the Images API has no corresponding CLI model anywhere, so "no capability surface" is the route's normal condition, not a reason to reject — the proxy's job is to come as close to the direct result as it can. Diagnostic use only; the default `codex-backend` transport sends every one of these on the tool. | Recorded 2026-08-29 when the tool-slot wiring landed on the default transport. Before that the options were restated to the model as prose on both transports, which is the mechanism the option rule forbids; the app-server path lost that prose without gaining a slot, because it has none to gain. Decided 2026-08-29: no blanket refusal. What code can realize on the returned bytes — `output_format`/`output_compression` conversion, `size` resizing, compositing the source through the `mask` — is the way to close the gap on this path, and is backlog. |
