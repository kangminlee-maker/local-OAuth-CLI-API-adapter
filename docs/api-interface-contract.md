...
| `prompt` | required | required | At most 32,000 UTF-16 code units (JavaScript string length); longer is 400. |
| `images` (JSON) / `image`, `image[]` (multipart) | ignored | required | A JSON edit names its images as `images`, an array of objects (`{image_url}` with a URL or data URL, `{b64_json}`, `{file_id}`); `image` or `image[]` in JSON is 400 `invalid_value` with the direct message (`use 'images' (array)`), a non-array `images` or a non-object member is 400 `invalid_type` naming what it got, an absent `images` is 400 `missing_required_parameter`. Multipart names its files `image` or `image[]`. |
| `mask` | ignored | optional | Sent to the backend as the `image_generation` tool's `input_image_mask` (a data URL for base64/path sources, the URL itself for URL sources); never attached as an input image. On the diagnostic `app-server` image transport there is no tool declaration: the mask is attached as one more reference image and nothing names it — see the "Images `app-server` transport" row. |
| `n` | optional | optional | Integer 1-10, default 1. |
| `size` | optional | optional | `auto` or `WIDTHxHEIGHT`. |
| `quality` | optional | optional | `standard`, `hd`, `low`, `medium`, `high`, `auto`. |
...
| `output_format` | optional | optional | `png`, `jpeg`, `webp`. |
| `output_compression` | optional | optional | Integer 0-100. Values below 100 require `jpeg` or `webp`; `100` (no compression) is valid with PNG. `null` is omission — the field is nullable on the direct API. |
| `moderation` | optional | optional | `low`, `auto`. Sent on the `image_generation` tool (`codex-backend` image transport only). |
...
| `stream` | optional | optional | Boolean. |
| `partial_images` | optional | optional | Only `0`, `null` or omitted is supported (`null` is omission — nullable on the direct API). Values above 0 return 400. |
| `x_proxy_image_route` | optional | optional | Proxy-only extension object. Multipart requests may pass it as a JSON string form field. |
