...
| `temperature` | number | Only the default `1` is accepted (null is omission). Any other value is 400 `invalid_request_error` / `param: temperature` / `code: unsupported_value` — the direct API's own envelope for this model family; no backend applies the value. |
| `top_p` | number | Only the default `1` is accepted (null is omission). Any other value is 400 `invalid_request_error` / `param: top_p` / `code: unsupported_parameter`. |
