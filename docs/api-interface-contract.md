...
| `output_compression` | optional | optional | optional | Integer 0-100. Values below 100 require `jpeg` or `webp`; `100` (no compression) is valid with PNG. Explicit `null` is rejected, as for `n`. |
...
| `partial_images` | optional | optional | optional | Only `0` or omitted is supported. Values above 0 return 400, and an explicit `null` is a value the client chose and is rejected, as for `n`. |
