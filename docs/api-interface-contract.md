...
| Images `app-server` transport | `--codex-image-transport app-server` delivers **no Images API option** to the model. That path starts a Codex app-server turn with the prompt and the attached images only — there is no `image_generation` tool declaration to carry `size`, `quality`, `output_format`, `output_compression`, `background`, `moderation`, `input_fidelity` or `input_image_mask`, and none of them is described in prose either. A `mask` is attached as one more reference image. The response still echoes the request's `background`, `size`, `quality` and `output_format`, none of which reached the model. Diagnostic use only; the default `codex-backend` transport sends every one of these on the tool. | Recorded 2026-08-29 when the tool-slot wiring landed on the default transport. Before that the options were restated to the model as prose on both transports, which is the mechanism the option rule forbids; the app-server path lost that prose without gaining a slot, because it has none to gain. Whether this transport should refuse the options it cannot deliver, or be retired, is an open decision. |
