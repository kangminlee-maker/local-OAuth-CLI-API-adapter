...
| Claude session user settings | By default the spawned Claude CLI loads the `user` setting source, so the operator's global CLAUDE.md bundle enters every proxied session — adding its tokens to each turn and letting personal instructions color API responses. `--isolate-user-settings` loads no setting source instead; OAuth auth is unaffected. | Default preserves pre-existing behavior. The opt-in exists because an API adapter's responses should look like the provider's, not like the operator's personal assistant (measured 2026-08-19: bundle added ~3.1k tokens and ~1s per message). |
