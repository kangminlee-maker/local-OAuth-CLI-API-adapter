...
| Claude session user settings | By default every spawned Claude child — the API backend and native chat sessions alike — loads the `user` setting source. That is the whole of `~/.claude/settings.json` plus the operator's global CLAUDE.md, so a proxied API request carries the operator's memory bundle into its context AND runs their configured hooks (`SessionStart`, `PreToolUse`, `Stop`, … — arbitrary local commands) once per turn, under their `env`, `permissions`, and model/effort defaults. Any local client that can reach the proxy port therefore triggers those hooks. `--isolate-user-settings` (bare, or `=true`/`=false`; any other value is a startup error, and the flag is rejected outside `--runtime claude`) passes `--setting-sources ''`, which loads none of it. Keychain OAuth is unaffected — verified on claude 2.1.235 — but a setup that supplies credentials *through the settings file* (`apiKeyHelper`, `env`) loses them under the flag, since isolation drops the whole source rather than the memory bundle alone. Omitting `--setting-sources` is NOT equivalent to the empty value: probed on 2.1.235, an omitted flag loads user settings exactly as `user` does. An operator `--extra-arg --setting-sources <value>` is appended after the isolation args and therefore wins, the same precedence `--extra-arg --model` already has. | Default preserves pre-existing behavior. The opt-in exists because an API adapter's responses should look like the provider's, not like the operator's personal assistant, and because hook execution per API request is a surprising authority for an HTTP surface to carry (measured 2026-08-19: the bundle added ~3.1k tokens and ~1s per message). |
