...
| Model catalog | With `modelSelection.honorRequestModel` off, `GET /v1/models` returns the single executed model. With it on, it returns what the runtime advertises: the Codex catalogue from `codex debug models`, or the aliases `claude --model` documents (`fable`, `opus`, `sonnet`). The executed model is listed first. In both modes the list is empty when the only value available is a backend identifier — that is, when no model is configured and the runtime advertises nothing. | The list is never hard-coded, so a new model generation appears without a code change. Codex entries are listed as advertised, including ones marked hidden or not API-supported, because validation accepts any advertised slug — the list and the gate agree. Claude version-pinned names (`claude-opus-5`, …) are not listed since the CLI does not enumerate them; a client may still name one and the CLI validates it. An empty list is the honest answer when nothing selectable is known: the alternative is advertising `codex-backend`/`claude-code-cli`, which the proxy would then reject as a model. |
