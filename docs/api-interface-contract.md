...
| `thinking` | object | `type` must be `adaptive`, `enabled`, or `disabled` — anything else is 400. `display` (`summarized`/`omitted`) is honored except with `disabled`, which never produces thinking blocks to display. Mapped to the Claude CLI's thinking flags. |
...
| `output_config.task_budget` | object | `{type: "tokens", total: <integer ≥ 20000>}`; a wrong `type`, a non-integer or an undersized `total` are each a named 400. Only absence is omission — an explicit `null` is a present wrong value, 400 (`format` and `thinking.display` follow the same rule; `effort` alone documents null-as-omission). |
| `temperature` | number | Passed to backend as temperature hint. |
...
| GPT image `response_format` | `response_format` is rejected for `gpt-image-*` models. On the models that accept it, a present value must be `url` or `b64_json` — a number, `null` or any other shape is 400, never silently defaulted. | Mirrors provider-style parameter compatibility for GPT image models. |
