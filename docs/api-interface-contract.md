...
| Text alongside tool calls | A turn that narrates and then calls a tool returns both, on every surface and in both modes: chat `content` carries the narration next to `tool_calls`, Responses output holds the message item before the `function_call` items, and `/v1/messages` content holds a `text` block before the `tool_use` blocks. | Both upstream APIs return them together, and the streamed deltas already delivered the narration — dropping it from the result made streaming and non-streaming clients disagree about what the model said. |
| Streaming wire positions | `response.output_index` and Anthropic `content_block` indices are wire positions, allocated in the order items and blocks are emitted. Every item holds a distinct index, and no block is stopped that was not started. | These indices are what SDK stream accumulators key on. Deriving a tool's index from its position among tools alone collided with the reasoning and message items when a turn both narrated and called a tool, and a tool block opening before the text block claimed an index the text block then reused. |
| Streamed tool identity | A tool call is announced to the client only once the backend has supplied the `call_id` the client must echo back with the result. Arguments that arrive before it are held and delivered with the announcement. | The identity in a stream is a promise: a client that has reported `fc_1` cannot rename it when the completed result says `call_1`, so it answers under an id the model never used. Waiting costs latency on a degraded stream; getting it wrong costs the turn. |
| Turn termination on `codex-backend` | A turn is reported as finished only when the backend says it finished. `response.failed`, `response.incomplete`, an SSE `error` frame, and a stream that ends with no terminal event are all failures, surfaced through the same paths as any other backend failure (an HTTP error before the first byte; an in-band error event once the stream has committed). Tool arguments that never arrive are an empty object, not a synthesized `input` property. | Previously any of these produced a 200 whose `content` was whatever had arrived and whose `finish_reason` was `stop`, so a failed or truncated generation was indistinguishable from a complete answer — including a dropped connection mid-tool-call, which surfaced as a well-formed call with fabricated arguments. |
