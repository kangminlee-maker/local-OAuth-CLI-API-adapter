...
| `mask` | ignored | optional | ignored | Sent to the backend as the `image_generation` tool's `input_image_mask` (a data URL for base64/path sources, the URL itself for URL sources); never attached as an input image. On the diagnostic `app-server` image transport there is no tool declaration: the mask is attached as one more reference image and nothing names it — see the "Images `app-server` transport" row. |
...
| `moderation` | optional | optional | optional | `low`, `auto`. Sent on the `image_generation` tool (`codex-backend` image transport only). |
| `input_fidelity` | invalid | optional | invalid | `high`, `low`; disabled for `image-2`. Sent on the `image_generation` tool for other models (`codex-backend` image transport only); the backend image model refuses it (`image_generation_user_error` / `invalid_input_fidelity_model`), and that rejection is forwarded. |
