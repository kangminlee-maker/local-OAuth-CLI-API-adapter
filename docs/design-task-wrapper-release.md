...
| separate | **landed 2026-09-04** — row 8's 502 at completion, and the validator the owner chose (`ajv` 8, draft 2020-12) for row 8 in full and row 10's `json_schema` half; a forced call the runtime reports as cut off by its output limit is kept, as the direct API keeps it. Row 13 measured (0 of 100 further turns) and declared. Row 2's 502 stays separate — the runtime's schema channel is not provider-forced, so a bare-prose `auto` answer is still delivered as prose | one commit; the validator is a dependency, the refusals are `parseBackendOutput` |
