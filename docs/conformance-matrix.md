...
| R-8 | `max_output_tokens` | integer | `null` (model max) | caps output; `status:"incomplete"` + `incomplete_details.reason:"max_output_tokens"` | 400 / `invalid_request_error` / `max_output_tokens` / — | parsed but **not enforced**; echoed in the body. **`incomplete_details` is hard `null` and `status` never becomes `incomplete`** → a capped turn is reported `completed`, where the direct API reports `status:"incomplete"` + `incomplete_details.reason:"max_output_tokens"` with `usage.output_tokens` exactly at the cap (measured 2026-09-02). No runtime channel exists — see row A-2 for the two that were tried | VERIFIED (measured 2026-09-02) |
...
| R-16 | `text.format` | `{type:"text"\|"json_object"\|"json_schema", name, schema, strict}` | `{"type":"text"}` | structured output | 400 / `invalid_request_error` / `text.format` / — | supported (`normalizers.ts:65-68`), through the same runtime schema as chat row 28 — `json_object` carries `{"type":"object"}` and non-object output is rejected; a client `json_schema` keeps its own root, is validated against that schema at the response boundary (chat row 28, §7 row 10) and is published with its numbers verbatim. **Not enforced together with `tools`**, same as chat row 28 | VERIFIED (`option-is-a-promise`, `wrapper-root-and-lexeme`) |
...
The schema the client supplied is the runtime's output contract, so an answer outside it is a
runtime that ignored its schema and the backstop refuses it — never repairs it. The validator is
`ajv` 8 (draft 2020-12, `strict` off because the schema is the client's, formats not asserted
because neither direct API asserts them on model output), the one runtime dependency taken since
`sharp`. A schema Ajv cannot compile judges nothing and the answer passes: an unverifiable promise
is not a broken one, and the direct APIs reject such a schema at request time.

| # | defect | what a client saw | what closed it |
|---|---|---|---|
| 8 | a forced single tool bypassed the wrapper entirely, and `normalizeToolArgumentsText` passed any `{`/`[`-opening payload through unvalidated | a truncated `{"city":"Seo` was published as `function.arguments` on Chat and Responses, and the two `/v1/messages` **writers** disagreed over it (buffered `{"input":…}` against a verbatim `input_json_delta`) | refused at completion, 502 `arguments that are not JSON`, on every surface and both paths — unless the runtime reports the output limit (`max_tokens` from Claude Code, `length` from the codex transport), when the fragment is kept as the direct APIs keep it under `finish_reason: "length"`. `app-server` reports no stop reason, so its fragment is always refused — **declared**, and not for want of looking: the app-server protocol schema (`codex app-server generate-json-schema`, codex-cli 0.153.0, dumped 2026-09-04) has `TurnStatus` = `completed | interrupted | failed | inProgress` and no length or token-limit reason anywhere on a turn or an agent message; the only `incomplete` in it is `LocalShellStatus`. Arguments that parse are validated against the tool's own `parameters` / `input_schema` and refused outside it (`outside the tool's schema`). `wrapper-agreement-suite` 7b-8 (refused, both backends) and 7b-8b (kept under the limit; the writers' disagreement pinned), `backend-contract.test` row 8 |
| 10 | with a JSON format present, a wrapper-shaped object with no usable `status` was exempted from refusal | `{"status":"done","text":"…","toolCalls":[]}` published verbatim — the proxy's internal grammar as the client's answer | `json_schema` **without tools**: the answer is validated against the client's schema and refused outside it (`outside the request's JSON schema`; text that is not JSON at all, `not JSON for a request that supplied a JSON schema`), which refuses the wrapper grammar along with every other near miss. `json_schema` **with tools**: chat row 28's declaration stands — the format never reached the runtime, so refusing its text would refuse every turn. `json_object`: **declared unenforced** beyond the object rule; nothing in the bytes separates wrapper from answer. `backend-contract.test` row 10, `what-the-reverted-member-left-behind` (the streaming gate now holds `json_schema` text too, `textMayBeRefused`) |
...
| # | defect | what a client saw | what closed it |
|---|---|---|---|
| 11 | a tool definition with a blank name was accepted, and Anthropic accepted one with **no** name at all | all three surfaces published a call literally named `tool` — `readString(…, 'tool')` in each declaration reader manufactured the identity the runtime enum and the backstop then trusted | 400 at the boundary, the direct envelopes verbatim. Chat: `empty_string` on `tools[i].function.name`. Responses: `empty_string` on `tools[i].name` (an absent name was already `missing_required_parameter`). Messages: `tools.i.custom.name: Field required` and `…: String should have at least 1 character` — the direct API reports the name under the `custom` member of its tool union. A non-string name on Messages answers `Input should be a valid string`, this validator's wording for `model`, **unmeasured** for this field. The readers' `tool` fallback is now reachable only through a Responses tool whose `type` is not `function` — a tool kind this proxy does not run, unmeasured, left as it was |
| 12 | a forced `tool_choice` naming a tool the request never declared created a call for it | with only `real` declared and `never_declared` forced, every surface published `never_declared` in both modes — `forcedSingleToolCall` fell back to a generic argument schema when the lookup found nothing, and this path bypassed `declaredToolNames` | 400 before `forcedSingleToolCall` can run, one check for the three shapes (`assertForcedToolDeclared` in `normalizers.ts`). Chat: `Invalid value for 'function_call': no function named 'X' was specified in the 'functions' parameter.`, `param: function_call` — the direct API still words it in the deprecated vocabulary, and on this model reaches that check only under `reasoning_effort: "none"`, which the parity row sets. Responses: `Tool choice 'X' not found in 'tools' parameter.`, `param: tool_choice`. Messages: `Tool 'X' not found in provided tools` |
