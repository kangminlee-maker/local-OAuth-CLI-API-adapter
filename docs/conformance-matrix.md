...
| 15 | `n` | integer ≥1 | `1` (caveat) | `choices` array length | 400 / `invalid_request_error` / `n` / — (and `n>1` unsupported on some models) | **validated** as the direct API validates it (integer, 1..8, measured bound) and **not yet realized**: one choice is returned. The fan-out is the next commit | VERIFIED for validation (§5.5.5); the choice count remains divergent |
