...
| R-20 | `parallel_tool_calls` | boolean | `true` | >1 call per turn | 400 / `invalid_request_error` / `parallel_tool_calls` / — | ignored, but **mirrored**: `false` comes back as `false` (`http-server.ts:2386`), which `conformance-supplied-echo.test` compares against the direct API's own echo. This cell read "echoed as a hard `true` even when the client sent `false`" until 2026-09-09, when the replay showed both sides sending `false` — the note had outlived the code | DOC |
