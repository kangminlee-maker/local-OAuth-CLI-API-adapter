...
| `body_temperature_*`, `body_top_p_0_5` | 요청 본문의 `temperature`(0.5, 1 모두)·`top_p` → 400 `{"detail": "Unsupported parameter: temperature"}`. 프록시는 보내지 않지만 `/v1/responses` 응답이 `temperature: request.temperature ?? 1`로 에코한다(`http-server.ts:2020`) — 적용된 적 없는 값의 에코 | VERIFIED |
