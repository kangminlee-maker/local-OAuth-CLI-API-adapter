...
| `WIRE` | The provider's own VALUE for **column 3** (what the provider answers), read from a promoted capture and re-read by a gate on every run, with the capture named. This is the class `VERIFIED` is explicitly not: what came back when the field was omitted, and what came back when it was supplied. A `WIRE` cell claims the bytes of the turn it cites and nothing more — not that the value holds for other models, other efforts, or other requests. It grades the provider side only: column 6 is this proxy's behaviour and is CODE-cited whatever the class says, so a `WIRE` grade never certifies the proxy note beside it. |
...
**Why `WIRE` exists, and what it cost to find out.** The grade was added on 2026-09-10 after four rows had measured values with nowhere to put them. `reasoning.context` was measured as `all_turns` on the wire and the row's grade was raised to `VERIFIED` and then put back, because the legend above scopes `VERIFIED` to an acceptance envelope and says so in the same sentence. A measured value is real evidence and it is not that evidence; writing it into the cell body while the column said `DOC` left the coverage instrument unable to count the very thing the probe plan exists to produce. The rule that follows: a `WIRE` cell must name a capture in `spec/captures/`, and a gate must replay that capture — a value read once and typed into a document is `DOC` at best, whatever it was read from. `test/conformance-manifest.test.mjs` checks both halves, because a review graded a row `WIRE` that named no capture at all and every instrument stayed green.
...
| 22 | `logprobs` | boolean | `false` (caveat) | populates `choices[].logprobs` — one entry per token with its `logprob`, its UTF-8 `bytes` and an empty `top_logprobs`, measured 2026-09-09 (`spec/captures/direct-chat-logprobs-effort-none.json`) | 400 / `invalid_request_error` / `logprobs` / — | **refused while the model reasons** (`unsupported_parameter`); at `reasoning_effort: none` it is accepted and not applied — the choice carries no `logprobs` key at all, declared in `spec/declared-divergences.json` and compared against the capture by `conformance-supplied-echo.test` | VERIFIED (§5.5.5) |
...
| 49 | `moderation` | `{model, policy:{input:{mode}, output}}` | absent | moderated output / refusals | 400 / `invalid_request_error` / `moderation` / — | validated as the direct API validates it (`moderation.model` required) and not applied — the local runtimes moderate nothing, so the response carries no `moderation` object. The direct answer is not empty: it is 200 **plus a 127-path `moderation` block** — `input` and `output`, each a `moderation_results` carrying `flagged`, 13 `categories`, 13 `category_scores` and 13 `category_applied_input_types`. Since 2026-09-10 that absence is **declared** (`chat-moderation-results-are-not-reported`) and replayed, not described: the values are a classifier's verdicts, not counters, so a `flagged: false` would be this proxy asserting the outcome of a check it never ran. **Unmeasured on the request side**: this proxy reads only `moderation.model` while the type above shows a nested `policy`, and the direct API is strict about nested members elsewhere — whether `{model, bogus}` is a 400 there and a 200 here is one free refusal probe and is not claimed either way | WIRE (`spec/captures/direct-chat-moderation-model.json`) |
...
| R-7 | `instructions` | string | `null` | system-level steering; echoed in `instructions` | UNKNOWN | supported; prepended as a `system` message (`normalizers.ts:48-51`) | WIRE (`spec/captures/direct-responses-instructions.json`) |
...
| R-9 | `max_tool_calls` | integer | `null` | caps built-in tool calls | UNKNOWN | ignored, but **mirrored**: the supplied integer comes back (`numberOrNull`, `http-server.ts:2382`), which is what the direct API does with it too. This cell read "echoed as hard `null`" against a stale line citation until 2026-09-10, when the capture below was replayed against the code | WIRE (`spec/captures/direct-responses-max-tool-calls.json`) |
...
| R-12 | `top_logprobs` | integer 0-20 | **`0`** on the wire (P-1), not the doc's `null` | logprobs on output text; the output-text content carries a `logprobs` member either way | 400 / `invalid_request_error` / `top_logprobs` / — | **refused while the model reasons** (`unsupported_parameter`, same sentence as `include`); at `reasoning.effort: none` accepted and echoed as supplied, and not applied — measured 2026-09-09 (`spec/captures/direct-responses-top-logprobs-effort-none.json`, replayed by `conformance-supplied-echo.test`). Until that capture this surface refused it at every effort while Chat accepted it at `none`: one proxy, two answers to one request. Where it is accepted it is held to the documented `0..20`; Chat's measured ceiling is 5 and which one the direct Responses API enforces is **unmeasured**, so the wider documented range is the one a request is held to. The first version of this change also opened `include`'s logprobs door and the two penalties at that effort, and a review shut them again: their acceptance here is unmeasured, and the refusal turned out to be the only validation in front of them — `presence_penalty: 99` was accepted and echoed back | WIRE (`spec/captures/direct-responses-top-logprobs-effort-none.json`) |
...
| R-17 | `text.verbosity` | `low`\|`medium`\|`high` | `medium` (**UNKNOWN**) | output length | 400 / `invalid_request_error` / `text.verbosity` / — | supported; the whole `text` object comes back with the vendor's own `format: {type: "text"}` beside the supplied verbosity, which the capture below shows both sides doing | WIRE (`spec/captures/direct-responses-text-verbosity.json`) |
...
| R-20 | `parallel_tool_calls` | boolean | `true` | >1 call per turn | 400 / `invalid_request_error` / `parallel_tool_calls` / — | ignored, but **mirrored**: `false` comes back as `false` (`http-server.ts:2386`), which `conformance-supplied-echo.test` compares against the direct API's own echo. This cell read "echoed as a hard `true` even when the client sent `false`" until 2026-09-09, when the replay showed both sides sending `false` — the note had outlived the code | WIRE (`spec/captures/direct-responses-tools-parallel-false.json`) |
...
| R-22 | `reasoning.summary` | `auto`\|`concise`\|`detailed` | `null`; **`auto` is echoed back as `detailed`** — the direct API resolves the alias (measured once, at `effort: low`, 2026-09-09, `spec/captures/direct-responses-reasoning-summary-auto.json`; that turn carried no summary either) | adds `summary` parts to the `reasoning` output item | 400 / `invalid_request_error` / `reasoning.summary` / — | **silently ignored** — the proxy never emits reasoning summaries; a client that asked for `detailed` gets an empty/absent summary with no error. The echo is not resolved either: `auto` comes back as `auto`, declared in `spec/declared-divergences.json` rather than mirrored, because one sample of one alias at one effort is not an alias table | WIRE (`spec/captures/direct-responses-reasoning-summary-auto.json`) |
| R-23 | `reasoning.context` | `auto`\|`current_turn`\|`all_turns` | **`all_turns`** — observed on the wire 2026-08-29 (P-1, `spec/captures/direct-responses-minimal.json`, whose request proves `reasoning` was omitted). The contract's own sample body showed `"context":"current_turn"`, which is where our echo's default came from until 2026-09-09; the sample is not the wire. The row's Ev is `WIRE`, the grade this row's own history produced: `VERIFIED` was tried here on 2026-09-09 and reverted, because this legend scopes it to "the field was accepted and the asserted response shape came back" and explicitly **never** to "the omitted default was X". Nothing here has measured the accepted values, the invalid-value error, or the effect — `WIRE` claims the capture's bytes and no more | which reasoning items replay | 400 / `invalid_request_error` / `reasoning.context` / — | echoed via `responseReasoning` at the vendor's measured default `all_turns`; not acted on (`conformance-echoed-defaults.test` compares the echo against `spec/captures/direct-responses-minimal.json`) | WIRE (`spec/captures/direct-responses-minimal.json`) |
| R-24 | `reasoning.generate_summary` | deprecated alias of `summary` | `null` | as R-22 | as R-22 | silently ignored | DOC |
| R-25 | `include` | array: `reasoning.encrypted_content`, `message.output_text.logprobs`, `web_search_call.action.sources`, `file_search_call.results`, `code_interpreter_call.outputs`, `computer_call_output.output.image_url`, `message.input_image.image_url` | `null` | adds the named payloads to `output` | 400 / `invalid_request_error` / `include` / — | `message.output_text.logprobs` is **refused at every effort** — 400 / `unsupported_parameter` / `include`, the door the direct API names when logprobs and reasoning are sent together (`normalizers.ts:1110`). Whether the direct API accepts the member at `reasoning.effort: none` is unmeasured on this surface, so no parity is claimed there and the refusal stands. Every other member is **silently ignored**: the answer carries no `include` key at all, measured for `reasoning.encrypted_content` — the one stateless multi-turn clients depend on — alone and alongside `store: false` | WIRE (`spec/captures/direct-responses-include-encrypted-content.json`) |
| R-26 | `store` | boolean | `true` on Responses (doc transcript echoes `"store":true`) | retrievable via `GET /v1/responses/{id}` | 400 / `invalid_request_error` / `store` / — | echoed (`raw.store === false ? false : true`, `http-server.ts:2401`) but **nothing is stored** — the proxy has no `GET /v1/responses/{id}`, so an echoed `store:true` is a false promise | WIRE (`spec/captures/direct-responses-store-false.json`) |
...
| R-30 | `truncation` | `auto`\|`disabled` | `"disabled"` (doc states default explicitly) | over-context: drop-from-start vs 400 | 400 / `invalid_request_error` / `truncation` / — | echoed, not acted on (`http-server.ts:2026`) | WIRE (`spec/captures/direct-responses-truncation-auto.json`) |
| R-31 | `metadata` | ≤16 pairs | `{}` (doc transcript echoes `{}`) | echoed | 400 / `invalid_request_error` / `metadata` / — | echoed via `asRecordPayload` (`http-server.ts:2418`) | WIRE (`spec/captures/direct-responses-metadata.json`) |
| R-32 | `user` | string (superseded) | `null` | cache bucketing | UNKNOWN | echoed (`http-server.ts:2032`) | WIRE (`spec/captures/direct-responses-user.json`) |
| R-33 | `safety_identifier` | string ≤64 | UNKNOWN (not in doc transcripts) | abuse bucketing | UNKNOWN | echoed (`http-server.ts:2017`) | WIRE (`spec/captures/direct-responses-safety-identifier.json`) |
| R-34 | `prompt_cache_key` | string | UNKNOWN | `usage.input_tokens_details.cached_tokens` | UNKNOWN | echoed (`http-server.ts:2014`) | WIRE (`spec/captures/direct-responses-prompt-cache-key.json`) |
...
| R-36 | `prompt_cache_options` | `{ttl, mode}` | `ttl:"30m"`, `mode:"implicit"` | explicit breakpoints | UNKNOWN | ignored, but **mirrored and canonicalised**: the vendor's own `mode: "implicit"` and `ttl: "30m"` are filled in under what the request sent (`http-server.ts:2393`), and the capture shows both sides answering the same object. Unlike the tool echo (declared, not canonicalised) this fills a cache hint rather than a guarantee about how the turn is run | WIRE (`spec/captures/direct-responses-prompt-cache-options.json`) |
| R-37 | `service_tier` | `auto`\|`default`\|`flex`\|`scale`\|`priority` | `"auto"` | echoed as tier used | 400 / `invalid_request_error` / `service_tier` / — | ignored, but **resolved and echoed**: `flex` comes back as `flex`, `fast` and `priority` both as `priority`, everything else as `default` (`resolvedOpenAiServiceTier`, reached from `http-server.ts:2400`). This cell read "hard `\"default\"`" until 2026-09-10; the capture it now cites shows the direct API answering `flex` to `flex`, and `conformance-supplied-echo.test` compares the two. A review found the false text still standing under the new grade. Two more branches are replayed since 2026-09-10 — `default` here, and `priority` on Chat | WIRE (`spec/captures/direct-responses-service-tier-flex.json`, `spec/captures/direct-responses-service-tier-default.json`) |
...
| R-39 | `context_management` | array of `{type:"compaction", compact_threshold}` | absent | auto-compaction of long contexts | UNKNOWN | silently ignored, and the answer carries no `context_management` key on either side | WIRE (`spec/captures/direct-responses-context-management.json`) |
| R-40 | `moderation` | `{model, policy}` | **`null`** — measured, not absent | moderated output | UNKNOWN | silently ignored; `moderation` echoed as hard `null` (`http-server.ts:2384`). This cell read "default when omitted: absent" and cited a line number that had moved, until 2026-09-10: all **19** promoted `/v1/responses` captures carry `moderation: null`, and `conformance-echoed-defaults.test` compares it on every run. What the vendor answers when a request DOES carry `moderation` on this surface is **unmeasured** — if it fills the field there, this hard `null` is a value divergence rather than a mirror | WIRE (`spec/captures/direct-responses-minimal.json`) |
...
| A-27 | `metadata.user_id` | opaque string | absent | abuse detection | 400 / `invalid_request_error` | validated as the direct API validates it — `user_id` only, and a string — then not applied. Not applied and **not echoed either**, which the replay compares: the direct API's own answer carries no `metadata` key back, so the two sides agree on the whole envelope (`spec/captures/direct-messages-metadata-user-id.json`, replayed by `conformance-supplied-echo.test`) | VERIFIED (§5.5.5-A; `spec/captures/direct-messages-metadata-user-id.json`) |
| A-28 | `service_tier` | `auto`\|`standard_only` | `"auto"` | **`usage.service_tier` in the response** ( `"priority"` / `"standard"` ) | 400 / `invalid_request_error` | validated (`Input should be 'auto' or 'standard_only'`) and not applied. `usage.service_tier` is **not emitted**, where the direct API always emits it: a local CLI runs in no tier, and the proxy reports the runtime's numbers rather than inventing one. That absence stopped being prose on 2026-09-10 — it is declared as `messages-vendor-routing-usage-is-not-reported` in `spec/declared-divergences.json` and replayed against the capture, so a build that started emitting a tier fails the gate instead of quietly agreeing with a paragraph (`spec/captures/direct-messages-service-tier-standard-only.json`) | VERIFIED (§5.5.5-A; `spec/captures/direct-messages-service-tier-standard-only.json`) |
...
| A-30 | `inference_geo` | string (beta) | UNKNOWN | inference region | 400 / `invalid_request_error` | validated (`must be one of ['global', 'us']`) and not applied; `usage.inference_geo` is not emitted, for the same reason as A-28 and under the same declaration — the capture shows the direct API echoing the REQUESTED geography back in `usage.inference_geo: "us"`, which is exactly the routing claim this proxy has no route to make (`spec/captures/direct-messages-inference-geo-us.json`) | VERIFIED (§5.5.5-A; `spec/captures/direct-messages-inference-geo-us.json`) |
...
| --- | --- | --- |
| `/v1/chat/completions` | ~~`usage.prompt_tokens_details.cache_write_tokens`~~ → **없음**(2026-09-09 채움), `moderation` 블록 전체(127경로 — 요청이 `moderation`을 실었을 때만; **선언**됨 `chat-moderation-results-are-not-reported`) | 없음 |
...
| --- | --- | --- | --- | --- | --- |
| **P-1** | A | `POST /v1/responses` `{"model":"gpt-5.5","input":"ping","max_output_tokens":16}` — every optional field omitted **except the output cap**, which the probe supplies to bound what it costs. That cap comes back as an echo of a supplied value, not as a default, and `conformance-echoed-defaults.test` counts the two apart; the branch that fills `max_output_tokens` when it is ABSENT is **unmeasured** | Full body. Every echoed default: `temperature`, `top_p`, `top_logprobs`, `truncation`, `store`, `parallel_tool_calls`, `service_tier`, `tool_choice`, `text`, `reasoning`, `background`, `max_tool_calls`, `metadata`, `prompt_cache_retention`, `billing`; and the exact key set | R-10, R-11, R-12, R-19, R-20, R-23, R-26, R-27, R-30, R-31, R-35, R-37 + §5.2 | 1 |
...
|---|---|---|---|
| 8 | a forced single tool bypassed the wrapper entirely, and `normalizeToolArgumentsText` passed any `{`/`[`-opening payload through unvalidated | a truncated `{"city":"Seo` was published as `function.arguments` on Chat and Responses, and the two `/v1/messages` **writers** disagreed over it (buffered `{"input":…}` against a verbatim `input_json_delta`) | refused at completion — 502 `arguments that are not JSON` (a forced answer is judged and published byte for byte, whitespace included; a BOM or U+00A0 is not JSON on either path — round 20), `not a JSON object` for a bare string, array or number, or `an \`arguments\` member that is not a string` for a wrapper call whose member is an object (the schema says string; re-serializing it rounded its numbers — round 20), since the direct APIs' function-call channel yields an object by construction, strict or not — on every surface and both paths of `claude` and `app-server`, for a forced call and for every call inside the wrapper alike (the reading used to REPAIR prose into `{"input": …}` on both paths, publishing an executable call the model never made — r18-codex; agreement-suite `7a-7` is now a refusal) — the `codex-backend` transport does not reach this reading: its native channel's arguments are the vendor's own answer, delivered verbatim (whitespace-only included; only the empty string is the direct API's `{}`) on Chat and Responses as the direct API delivers them (whose documentation says they may not be valid JSON) — round 21 removed the `{"input": …}` wrap the transport used to put around a completed call that did not parse, which the stream had never applied — and a call it reports cut off is delivered verbatim (round 18). On `/v1/messages`, where `input` is a JSON value, a completed native call whose arguments are not a JSON object is refused with the same 502 as the wrapper reading (the stream: the bytes, then an in-band `error`); whether the live vendor ever produces one is **unmeasured**. Inside a turn the vendor reports cut off nothing is judged: the call the cut hit — the turn's last call with nothing after it, the one whose block the stream leaves open (`cutCallLeftOpen`) — is projected (its complete members; `{}` for whole JSON that is no object); when narration follows the last call the cut hit the narration, and that call, like every other call of the turn, goes out as written where its bytes are JSON (the stream closed its block on them; r24-codex read the earlier sentence as promising the projection there) — the stream closed its block on them — and as the projection where they are not, since `input` must be a JSON value (round 23, **unmeasured** shape). A completed value that contradicts what the transport streamed for the call (a vendor contradicting itself; **not observed** on the live backend — 16 calls on `codex-backend`/`gpt-5.5`, four of them spelled to break a re-serialiser (trailing zeros, an exponent, unescaped non-ASCII and solidus, non-alphabetical keys), every one byte-equal to the bytes streamed, so the systematic mechanism is ruled out while a rare model-side revision is not: `review-artifacts/stage2/report.md` M10, 2026-09-09) reaches the body while the stream keeps the bytes it sent and closes on them — declared, not repaired (r23-fable F2); a completed item whose `id` and `call_id` name two different announced calls (item ids swapped) is left alone, the streamed pairing winning on both paths (r26-codex F2). Rounds 26–28 closed the native transport's finish-state gaps: the vendor's finish event fixes a call's value (later deltas dropped on every path, identified or not) while the signal that closes a surface's block waits for an event at a HIGHER output position (the vendor moved on; a late frame for an earlier item is not that — r28-fable F2; the vendor's progress is retained, so a call that learns its position or finishes after the vendor has moved past it is released on that frame, not at the terminal — r30-codex; progress advances from a tool event only once correlated and only at the call's accepted position — a duplicate frame for the cut call mislabelled with a higher position closed the block `response.incomplete` had to leave open — r31-codex F5; such a frame is refused since round 45) or for `response.completed`, so a call finished inside a cut-off turn keeps its bytes and its open block; a completed value may extend a finished fragment only while that signal has not gone out — once the block is closed the fragment stands on every path, and Messages refuses it as not a JSON object (r28: fable F1, codex F1); a call is announced on a `call_id` AND a name, and a call the vendor never names — or never gives a `call_id` — is refused at completion (HTTP 502 while no stream byte has been written, an in-band `error` once the stream has committed): half an identity is no identity, so neither the placeholder `tool` nor an item id or a minted `call_N` reaches a client, whatever tools the request declares (r27-fable F1, r27-codex); a present `arguments` member that is not text is refused wherever it arrives (finish event, closed item, completed output, delta) instead of being read past into `{}` (r27-codex), and so is a present identity member — `id`, `call_id`, `name`, `item_id` — that is not text, which presence alone let through as an object-valued id on every surface (r29-codex); only an event carrying an `output_index` claims that position — an identified event without one used to claim `#0`, and a later call really at position 0 was bound to it and vanished (r29-codex) — and a call bound by id on an index-less frame learns its position from the first later frame that carries one, so its finish signal still fires when the vendor moves on (r29-fable F1), and adopts the anonymous holder already at that position as its own prefix — live, or from the completed output when no id frame ever carried one (r30-fable F1; a holder that cannot be ordered against bytes the call already carries is refused, `cannot place`); a known call claims only its accepted position — a later frame naming another one claimed that too, and the real call there vanished (r30-codex); a frame naming a `call_id` and an `item_id` bound to two states folds them into the one the client knows, so a split arrival is one call, not two (r30-codex) — the completed item too, when one side is a state the client was never told about (r31-fable F2); a call the fold identifies is announced on that frame, or at the terminal frame ahead of its finish signal — a finish signal for a call the client was never told about corrupted every stream (r31-fable F1); the fold's survivor is chosen by the one rule wherever two states meet, a position claim included — absorbing only from the caller's side refused the turn whose holder was the call the client knew (r31-codex F3) — and a retired state's position alias survives only at the survivor's accepted position, or the real call arriving at the other position was bound to the survivor and vanished (r31-codex F4); whether the completed output's calls align by position with the streamed ones is judged per item against the streamed calls still standing — a count taken once before the loop went stale when a fold retired a state, and the anonymous item completing another call was discarded, that call going out as its fragment (r32-fable F1); an id-less completed item is placed by its array position — the state holding that `output_index` — before any dense count: placed by arrival order, two calls announced in reverse had their completed arguments swapped, each a valid extension of the shared `{` prefix (r31-codex F6); a call is announced when a frame goes out to the client, not when its identity is frozen — a state the completed output created counted as announced, its finish signal went out at wire index -1 and every writer announced it a second time from the result (r32-codex); the completed output is read in two passes — the items that name a call first, folds included, then the anonymous ones: each placed by the call holding its position whatever the two views count, and the rest aligned one to one with the standing calls no item placed, only while the views agree — judged per item in array order, an anonymous item before the fold was still discarded by the count the fold had yet to correct, and the arrival-order slot booked a call a position had already placed and stranded the position-less one (r33-fable F1, F2); the vendor's progress counts a tool event's position only for a call that can no longer fold into another — an item-id half at a later position, absorbable, advanced it and closed the block of the cut call it then folded into, a `content_block_stop` the direct stream never sends (r33-codex F3); the count that gates the aligned rest is the remainders' — the items no position placed against the standing calls no item placed — not the whole: an item adding a call the stream never showed outnumbered the standing calls, and the item completing the index-less streamed call was discarded (r34-fable F1); and when the remainders disagree each item left is a call of its own, reported without an identity and refused at completion — discarded instead, two calls the runtime wrote vanished under a 200 on every surface (r34-codex); the count pairs only when exactly one item and one call remain and the names agree — two to two in arrival order handed each call the other's arguments behind a shared `{` prefix, which the prefix gate cannot see (r35-codex) — and an anonymous item naming a different tool than the call at its position is refused as two calls named as one (kept out until round 41); item ids, call ids and output positions are separate namespaces — kept in one map, a `call_id` spelled `#0` resolved the holder of position 0 and a spelling shared across one call's item id and another's `call_id` bound the second call to the first, erasing it under a 200 (r35-codex); a known call claims only its accepted position from the completed output too — an item at another index naming it adopted the anonymous holder there, and the call went out with that position's arguments under a 200; the holder now stays a call without an identity, refused (r36-codex); and the one remaining pair keeps the same rule — the standing call pairs with the item at its accepted position or, having none, at any; the one item at another index handed the call that index's arguments under a 200 (r37-fable); so an anonymous item that correlates with nothing — no call at its position, no position-compatible remaining pair — is a call without an identity and the turn is refused, where the r26 fixture used to deliver the streamed calls — two complete calls streamed at positions 1 and 2, the completed output listing them anonymously at indices 0 and 1: the item at index 1 is the call holding that position and applies as a no-op, the item at index 0 correlates with nothing (no call at 0; the one call left sits at 2) and the turn is refused (declared change, round 37); a name is a witness every correlation that joins two pieces hears — a call known by id does not adopt a holder of another name, a named event at a position held by another name is refused at that frame, a split identity whose halves carry different names does not fold, a completed item does not take a holder of another name, and neither does the dense slot (it re-adopted the holder the two routes above had declined — r39-fable): bound or folded anyway, one call's arguments went out under the other's name on every surface (r37-codex); a call once named keeps that name, announced or not — before the announcement the last name won (r39-fable) — and a frame or completed item naming another tool for a known call is refused: kept out, the announced name stayed while the renamed item's arguments went out under it (r39-codex; rounds 23–25 delivered — declared change); an identified completed item at an index other than its call's accepted position is refused as arguments the transport cannot place (rounds 26–39 completed the call by its ids wherever the array listed it; r41-codex), and so is a live frame naming a known call at another position — it adopted nothing there while its bytes were delivered under the call (r45-codex) — and a frame or item joining two states at two accepted positions, whichever survives (folded, the retired position's bytes moved under the survivor's call — r45-codex), and two calls the client knows placed at one position by a frame or an item (left apart as declared, both went out as executable calls at two wire indices from the one backend position — r46-codex); an item at a position a call holds is that call when its name and `call_id` agree — an unfamiliar item id binds as on a live frame (refused at completion as a call missing its `call_id` while the live equivalent was accepted — r45-codex) — and is refused as two calls named as one otherwise (another `call_id` there minted a second call at the one position — r45-codex; the dense-slot route, which adopted position 1's bytes for the item at index 0 — r39-codex — is gone, the holder at the item's own index being the one door); every id a completed item carries names the call from then on, and an item resolving to a call an earlier item already placed is that call listed twice, refused — unbound, a second item carrying the supplied `call_id` minted a second call under the same client identity, two executable calls with one id (r37-codex) — with no exception since round 45: the split identity meeting (r32 — a second listing folding a streamed state into the call, with no value or the call's own; through that door another value replaced the first listing's under a 200 — r39-fable, r39-codex — and its equality was argued round by round: raw bytes, then the published value, `""` and `{}` being the one empty call on a completed turn but not on a cut one — r44-fable, r45-fable) was one `call_id` at two indices — the first listing created the call without a position, the second inherited the folded holder's — moving the call across the array: that call listed twice, refused (r45-codex); placement follows the fold's survivor — tracked by ordinal, the flag stayed on the state a later item folded away, and the second listing resolving to the survivor slipped past the gate with another value (r40-fable); the name is heard on the fold's survivor, after the fold — checked against the known state alone, an unnamed call adopting a `beta` holder took that name past a frame that said `alpha`, live and from the completed output (r42-fable); and the meeting exception counts every fold placing the item performed, so a listing folding the holder in with the call's own value is one call and its other-value twin is refused (r42-fable; since round 45 no listing meets a placed call — the door `adoptHolder` opened and the anonymous item at a placed call's position r41-fable had gated are unreachable under strict positions, the two-id fold is refused, and the meeting exception and the fold counter are gone); three lanes rounds 23–39 had declared are refusals now (r41-codex, all on a/): an anonymous item naming another tool than the call at its position (kept out, a call the runtime wrote vanished under a 200), an identified item listed at an index other than its call's accepted position (correlated by its ids, the answer was repaired into the stream's order — the wire-positions row decides: `output[i]` is the item announced at `output_index: i`; the round-26 fixtures that reordered the array are reshaped to the live positions), and a frame or item naming another `call_id` for a known call (the announced id was kept and ratified with a 200 — the first `call_id` is latched like the name, at every door — the position-resolved door heard only the name, and a frame at the call's position naming another `call_id` had its arguments ratified under the latched id (r45-fable); and no item id stands in for it — the added frame's item-id fallback looked only at `identified`, a `call_id` AND a name, so a name-bearing frame joining a fold's survivor that owned the `call_id` but no name yet put its item id where the `call_id` stood, and every surface told the client to echo an id the backend never supplied as one — r43-codex); a cancellation that never settles no longer holds back a decided outcome, refusal or completed answer (r30-codex); a refusal raised while the body is open is the turn's outcome, not the unread body's teardown rejection (r29-fable F2); an argument event that names neither an item nor an output position is refused rather than read as position 0 and spliced into the next call (r27-codex); a request that permits no call — no tools declared, or `tool_choice: none` — refuses every call the vendor makes at its FIRST tool event, before anything is announced (refusing at completion published the call on the stream first — r29-codex), an empty allowed set rather than no rule (r28-codex F4); a finish event without its `arguments` member finishes nothing but still names its call and position, so the finished calls below it are announced on it (r29-codex); a present non-text `arguments` member on `output_item.added` is refused too (validated, not consumed — whether the added snapshot's string is cumulative is **unmeasured**; r29-codex); a completed item carrying a `call_id` for a call the stream identified by its item id alone binds to that call at its output position instead of minting a second one (r28-codex F7); a call the vendor names only after narration that followed it is reported after that narration on both paths — the order the client was told (declared; r28-codex F5); anonymous argument deltas belong to the call the completed output names at their position; the terminal frame ends the read; and stream tool indices are announcement order, the order `toolCalls()` reports, so a call identified late keeps the block the stream opened for it. Unless the runtime reports the output limit (`stop_reason: max_tokens`, the one spelling both readers use): then the turn is delivered **whatever it holds** — a JSON fragment, a parseable partial object, `{}`, a `required` turn with no call — as the direct APIs deliver it (a frame that breaks the protocol is still refused inside a cut turn — half an identity, a non-text member, an event naming no call: those refusals judge the vendor's frames, not the call's value — r29) (`review-artifacts/stage2/report.md` M6/M7), and the terminal fields say so: Chat `finish_reason: length`, Responses `status: incomplete` + `incomplete_details` + an incomplete item + `response.incomplete`, Messages `stop_reason: max_tokens` with the `tool_use` block — whose body carries the complete top-level members parsed so far (`completeTopLevelMembers`, a walk over the completed fragment) and whose stream carries the verbatim fragment in a block that is not closed — the turn's FINAL block only, and only when the fragment is not a whole JSON object: a last call whose bytes are a complete object (`{"a":1}` under `max_tokens`) is closed like any other, and what the direct API does with a complete object cut off at the limit is **unmeasured** (r23-codex); a cut call with narration or another call after it closes like any other, or the next block would nest (round 20) — exactly the direct API's two projections (measured 2026-09-04). One cut-off shape is refused rather than delivered: a turn cut off **inside the tool wrapper** (the wrapper JSON itself never closed), since the fragment is this proxy's grammar and publishing it as the assistant's words is the row-10 defect — 502 `cut off at its output limit inside the tool wrapper`, **declared**, unreachable on the live runtimes (neither reports the cap). The Claude CLI errors at its own cap rather than truncating, so on the live `claude` runtime this path is the double's; `app-server` reports no stop reason (its protocol has none), so its fragment is always refused — **declared**. Arguments that parse are validated against the tool's schema only under `strict: true`, the promise the direct APIs themselves enforce; without it they are delivered (round 18). `wrapper-agreement-suite` 7b-8/7b-8b, `backend-contract.test` rows 8 + r18, `output-limit-projection.test`, `codex-backend-transport.test` (fragment verbatim) |
