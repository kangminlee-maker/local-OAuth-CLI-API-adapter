...
| `WIRE` | The provider's own VALUE for this cell, read from a promoted capture and re-read by a gate on every run, with the capture named. This is the class `VERIFIED` is explicitly not: what came back when the field was omitted, and what came back when it was supplied. A `WIRE` cell claims the bytes of the turn it cites and nothing more — not that the value holds for other models, other efforts, or other requests. |
...
| R-26 | `store` | boolean | `true` on Responses (doc transcript echoes `"store":true`) | retrievable via `GET /v1/responses/{id}` | 400 / `invalid_request_error` / `store` / — | echoed (`raw.store === false ? false : true`, `http-server.ts:2019`) but **nothing is stored** — the proxy has no `GET /v1/responses/{id}`, so an echoed `store:true` is a false promise | WIRE (`spec/captures/direct-responses-store-false.json`) |
...
| R-31 | `metadata` | ≤16 pairs | `{}` (doc transcript echoes `{}`) | echoed | 400 / `invalid_request_error` / `metadata` / — | echoed via `asRecordPayload` (`http-server.ts:2033`) | WIRE (`spec/captures/direct-responses-metadata.json`) |
...
| R-37 | `service_tier` | `auto`\|`default`\|`flex`\|`scale`\|`priority` | `"auto"` | echoed as tier used | 400 / `invalid_request_error` / `service_tier` / — | ignored; hard `"default"` (`http-server.ts:2018`) | WIRE (`spec/captures/direct-responses-service-tier-flex.json`) |
