...
6. **CLI가 설치되지 않은 머신에서 smoke가 실패한다.** 모집단이 비는 것을 "파서가 멈췄다"로 읽기 때문이다.
7. **예산 소진 후 컨트롤 probe가 "CLI 거부 문구가 바뀌었다"고 보고한다.** 실제 원인은 스스로 멈춘 예산인데,
   문서화된 대응은 matcher를 새 릴리스에 맞춰 다시 유도하는 것이라 가장 시끄러운 오진이다.
8. **`budgetExhausted` 필드는 아무도 읽지 않는다.** 소진과 실제 CLI 실패를 구분할 수 있는 유일한 신호인데
   소비자가 없다.
