...
- 미결 설계 결정: 규약 위반 요청(5.6 + chat tools + effort 미지정)을 direct는 400으로 거절하지만 proxy는 Responses 변환 경로라 통과시킨다. proxy가 이 오류 표면을 미러링할지는 runtime 설계 결정으로 남아 있다(error-parity negative row는 결정 후 추가).
