...
| Codex image transport | `codexProxy.imageTransport` defaults to `codex-backend`; Images API uses backend `image_generation` tool results, with every option sent on the tool. The diagnostic `app-server` transport has no tool declaration, so it realizes `size`, `output_format`/`output_compression`, `background: opaque` and `mask` on the returned bytes (`sharp`, the package's one runtime dependency, loaded only there) and answers `transparent`/`input_fidelity` with the backend image model's refusal. |
